...
const char* get_pos (char *phones, int index)
{
	int pau_index;
	const char *result;
	...
	if (pau_index < 4) {
	    ...
	    phones[pau_index - 1] == PHONE_A && 
	    phones[pau_index - 2] == PHONE_N &&
	    phones[pau_index - 3] == PHONE_V) {
		result = "sname";
	} else if (
	    phones[pau_index - 1] == PHONE_V && 
	    (phones[pau_index - 2] == PHONE_O || phones[pau_index - 2] == PHONE_E)) {
		result = "surname-ovev";
	} else if (
	    phones[pau_index - 1] == PHONE_A && 
	    ...
	    phones[pau_index - 1] == PHONE_C && 
	    phones[pau_index - 2] == PHONE_N &&
	    phones[pau_index - 3] == PHONE_A) {
		result = "surname-yan";
	} else if (
	    phones[pau_index - 1] == PHONE_N && 
	    phones[pau_index - 2] == PHONE_A) {
		result = "surname-yan";
	} else if (
	    phones[pau_index - 1] == PHONE_O && 
	    phones[pau_index - 2] == PHONE_K) {
		result = "surname-ko";
	} else if (
	    phones[pau_index - 1] == PHONE_K && 
	    phones[pau_index - 2] == PHONE_U) {
		result = "surname-uk";
	} else if (
	    phones[pau_index - 1] == PHONE_IJ && 
	    phones[pau_index - 2] == PHONE_I && 
	    ...
	    phones[pau_index - 1] == PHONE_H && 
	    (phones[pau_index - 2] == PHONE_I || phones[pau_index - 2] == PHONE_Y)) {
		result = "surname-ih";
	} else if (
	    phones[pau_index - 1] == PHONE_N && 
	    (phones[pau_index - 2] == PHONE_I || phones[pau_index - 2] == PHONE_Y)) {
...
ask_question (char *phones, int index, int tree_index)
{
	...
	switch (ru_stress_cart_nodes[tree_index].type) {
		...
		return (strcmp(phoneset[phones[index] - 1].vfront, 
			ru_stress_cart_nodes[tree_index].value_string) == 0);
	    case CART_QUESTION_PH_VHEIGHT:
...
find_stress_probability (char *phones, int index, int tree_index)
{
    ...
    if (ask_question (phones, index, tree_index)) {
	    ...
	    return find_stress_probability (phones, index, ru_stress_cart_nodes[tree_index].no_index);
    }
    return 0.0;
}
